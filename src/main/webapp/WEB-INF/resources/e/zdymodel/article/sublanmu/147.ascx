...
<%if(IsSearch==0){conn.Open();%><ul class="mk_wzlbzbrq_box">
<% 
...
%>
  <li>
    <span class="date"><em><%=((DateTime)dr["thedate"]).ToString("dd")%></em><%=((DateTime)dr["thedate"]).ToString("yyyy-MM")%></span>
    <div class="con">
       <span class="title"><a href="<%=Detail_Url(dr)%>"><%=dr["title"].ToString()%></a></span>
       <div class="jj"><%=SubStr(dr["pa_introduct"].ToString(),150,true)%></div>
    </div>
  </li>
<%
 }
