...
<%if(IsSearch==0){conn.Open();%><ul class="new_pic">
<% 
...
%>
<li class="xinwens"><a href="<%=Detail_Url(dr)%>"><div class="news_pics"><img src="<%=dr["titlepic"].ToString()%>"></div><div class="news_coms"><span class="s1"><%=dr["title"].ToString()%></span><span class="s2"><%=SubStr(dr["pa_introduct"].ToString(),150,true)%></span></div></a></li>
<%
 }
%>
</ul><%conn.Close();}
