...
<%if(IsSearch==0){conn.Open();%><ul class="zy_piczs">
<% 
...
%>
<li class="zspic"><a href="<%=Detail_Url(dr)%>"><span class="s1"><img src="<%=dr["titlepic"].ToString()%>"></span><span class="s2"><%=dr["title"].ToString()%></span></a></li>
<%
 }
%>
</ul><%conn.Close();}
